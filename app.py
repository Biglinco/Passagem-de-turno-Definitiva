    ...
    with tab1:
        ...
        if st.session_state['fornecedores']:
            ...
            for chave, dados in list(st.session_state['fornecedores'].items()):
                ...
                with col1:
                    st.write(f"- **{dados['fornecedor']} ({dados['transportadora']})**: {dados['paletes']} paletes | {dados['checklists']} checklists")
                with col2:
        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if st.session_state['fornecedores']:
                ...
                for chave, d in st.session_state['fornecedores'].items():
                    relatorio += f"▫️ {d['fornecedor']} (Transp: {d['transportadora']}): {d['paletes']} paletes | {d['checklists']} cklts Feitos\n"
            ...
            if HAS_PIL:
                ...
                # Paleta de Cores (Estilo Cimed: Preto e Amarelo)
                COR_CABE_FUNDO = (20, 20, 20)      # Preto profundo p/ fundo cabeçalho
                COR_DESTAQUE = (255, 215, 0)       # Amarelo/Dourado Cimed
                COR_AZUL_CLARO = (245, 245, 245)   # Fundo base do documento quase branco/cinza
                COR_BRANCO = (255, 255, 255)       # Caixas
                ...
                if st.session_state['fornecedores']:
                    for chave, d in st.session_state['fornecedores'].items():
                        txt_forn += f"• {d['fornecedor']} (Transp: {d['transportadora']}): {d['paletes']} Paletes | {d['checklists']} CKL\n"
                else:
                ...
                # ===============================
                # DESENHANDO CABEÇALHO SUPERIOR (Preto Cimed)
                # ===============================
                d.rectangle([(0, 0), (LARGURA_IMG, 140)], fill=COR_CABE_FUNDO)
                
                # Textos cabeçalho - amarelo/ouro nos títulos
                centro_x = LARGURA_IMG // 2
                d.text((40, 40), "ALMOXARIFADO", fill=COR_BRANCO, font=fonte_secao)
                d.text((centro_x, 30), "Passagem de Turno", fill=COR_DESTAQUE, font=fonte_titulo_grande, anchor="ma")
                d.text((centro_x, 80), "RELATÓRIO DIÁRIO DE ATIVIDADES", fill=COR_BRANCO, font=fonte_subtit, anchor="ma")
                d.text((LARGURA_IMG - 40, 40), f"Gerado em: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", fill=COR_DESTAQUE, font=fonte_rodape, anchor="ra")
                ...
                for titulo, linhas_texto in linhas_sessao:
                    ...
                    # Faixa Titulo da Sessão (Preto)
                    d.rectangle([(margem_esquerda, y_atual), (margem_esquerda + largura_quadro, y_atual + 40)], fill=COR_CABE_FUNDO)
                    # Titulo da Barra em Amarelo Destaque
                    d.text((margem_esquerda + 20, y_atual + 8), titulo, fill=COR_DESTAQUE, font=fonte_secao)
            ...
            for chave, d in st.session_state['fornecedores'].items():
                writer.writerow([data_hora_str, 'FORNECEDOR', d['fornecedor'], d['transportadora'], '', '', d['paletes'], d['checklists'], ''])
