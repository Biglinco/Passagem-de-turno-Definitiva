        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if HAS_PIL:
                ...
                for titulo_sessao, texto_sessao in sessoes:
                    ...
                    altura_conteudo += 280 + (len(linhas) * 44) 
                ...
                d.text((LARGURA_IMG - 80, 80), f"Gerado em: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", fill=COR_CABE_TEXTO, font=fonte_rodape, anchor="ra")
                ...
                try:
                    d.rounded_rectangle([(80, y_capsula), (780, y_capsula+90)], fill=COR_BRANCO, radius=16)
                    # Aumentar a largura da caixa 2 (Horário) e empurrar a caixa 3 mais pra direita
                    d.rounded_rectangle([(810, y_capsula), (1280, y_capsula+90)], fill=COR_BRANCO, radius=16)
                    d.rounded_rectangle([(1310, y_capsula), (LARGURA_IMG-80, y_capsula+90)], fill=COR_BRANCO, radius=16)
                except AttributeError:
                    d.rectangle([(80, y_capsula), (780, y_capsula+90)], fill=COR_BRANCO)
                    d.rectangle([(810, y_capsula), (1280, y_capsula+90)], fill=COR_BRANCO)
                    d.rectangle([(1310, y_capsula), (LARGURA_IMG-80, y_capsula+90)], fill=COR_BRANCO)
                ...
                # Caixinhas de informações gerais de Cabeçalho (negritos base na label) x2
                d.text((110, y_capsula+24), f"Dia da Semana: ", fill=COR_CABE_TEXTO, font=fonte_label)
                tam1 = d.textlength("Dia da Semana: ", font=fonte_label)
                d.text((110 + tam1, y_capsula+24), dia_semana_str, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                
                d.text((830, y_capsula+24), f"Horário de Geração: ", fill=COR_CABE_TEXTO, font=fonte_label)
                tam2 = d.textlength("Horário de Geração: ", font=fonte_label)
                d.text((830 + tam2, y_capsula+24), hora_str, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                
                # Deslocado 1300 para repozicionar dentro da nova caixa
                d.text((1340, y_capsula+24), "Sistema: ", fill=COR_CABE_TEXTO, font=fonte_label)
                tam3 = d.textlength("Sistema: ", font=fonte_label)
                d.text((1340 + tam3, y_capsula+24), "Web App", fill=COR_TEXTO_ESCURO, font=fonte_texto)
