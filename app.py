    ...
    with tab5:
        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if HAS_PIL:
                ...
                # Removemos o @st.cache_data porque o Streamlit não consegue fazer o "pickle"
                # (salvar na memória cache) de objetos C da biblioteca truetype/FreeType com segurança na nuvem.
                def get_font(size):
                    try:
                ...
                if st.session_state['fornecedores']:
                    for chave, d in st.session_state['fornecedores'].items():
                        forn = d.get('fornecedor', chave)
                        transp = d.get('transportadora', 'N/I')
                        txt_forn += f"• {forn} (Transp: {transp}): {d['paletes']} Paletes | {d['checklists']} CKL\n"
                else:
