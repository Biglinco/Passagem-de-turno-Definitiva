...
def main():
    ...
    with tab4:
        ...
        st.info("Registre a contagem de paletes, veículos aguardando e pendências gerais.")
    ...
    with tab5:
        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if HAS_PIL:
                import textwrap
                try:
                    # Tenta carregar fontes variadas do sistema (no Windows)
                    fonte_titulo_grande = ImageFont.truetype("arialbd.ttf", 36)
                    fonte_subtit = ImageFont.truetype("ariali.ttf", 20)
                    fonte_secao = ImageFont.truetype("arialbd.ttf", 22)
                    fonte_label = ImageFont.truetype("arialbd.ttf", 16)
                    fonte_texto = ImageFont.truetype("arial.ttf", 16)
                    fonte_rodape = ImageFont.truetype("ariali.ttf", 14)
                except IOError:
                    # Fallback
                    fonte_titulo_grande = ImageFont.load_default()
                    fonte_subtit = ImageFont.load_default()
