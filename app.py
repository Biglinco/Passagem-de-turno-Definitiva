    ...
    with tab5:
        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if HAS_PIL:
                ...
                fonte_titulo_grande = get_font(36, bold=True)
                fonte_subtit = get_font(20, bold=False)
                fonte_secao = get_font(22, bold=True)
                fonte_label = get_font(16, bold=True)
                fonte_texto = get_font(16, bold=False)
                fonte_rodape = get_font(14, bold=False)

                # Paleta de Cores (Estilo Cimed: Amarelo e Preto)
                COR_CABE_FUNDO = (245, 210, 30)    # Amarelo Ouro p/ fundo cabeçalho e faixas de título
                ...
                for titulo_sessao, texto_sessao in sessoes:
                    ...
                    # Matemática ajustada para não cortar texto: (espaçamento+padding extraizado)
                    altura_conteudo += 140 + (len(linhas) * 22) 
                
                altura_total = 250 + altura_conteudo + 100 # Cabeçalho(250) + conteudo + Rodape aumentado(100)
                LARGURA_IMG = 850
                ...
                d.text((40, 40), "ALMOXARIFADO", fill=COR_CABE_TEXTO, font=fonte_secao)
                d.text((centro_x, 30), "Passagem de Turno", fill=COR_CABE_TEXTO, font=fonte_titulo_grande, anchor="ma")
                d.text((centro_x, 80), "RELATÓRIO DIÁRIO DE ATIVIDADES", fill=COR_CABE_TEXTO, font=fonte_subtit, anchor="ma")
                d.text((LARGURA_IMG - 40, 40), f"Gerado em: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", fill=COR_CABE_TEXTO, font=fonte_rodape, anchor="ra")
                ...
                # Pega a hora AGORA para evitar cache de horário fixo em servidor
                agora = datetime.datetime.now()
                dia_semana_str = dias_pt[int(agora.strftime('%w'))]
                hora_str = agora.strftime('%H:%M')
                
                # Caixinhas de informações gerais de Cabeçalho (negritos base na label)
                d.text((55, y_capsula+12), f"Dia da Semana: ", fill=COR_CABE_TEXTO, font=fonte_label)
                # Dando espaço e imprimindo o resto em texto normal
                tam1 = d.textlength("Dia da Semana: ", font=fonte_label)
                d.text((55 + tam1, y_capsula+12), dia_semana_str, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                
                d.text((415, y_capsula+12), f"Horário de Geração: ", fill=COR_CABE_TEXTO, font=fonte_label)
                tam2 = d.textlength("Horário de Geração: ", font=fonte_label)
                d.text((415 + tam2, y_capsula+12), hora_str, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                
                d.text((635, y_capsula+12), "Sistema: Web App", fill=COR_CABE_TEXTO, font=fonte_label)
                ...
                for titulo, linhas_texto in linhas_sessao:
                    # Bloco Fundo da sessão (branco) com mais pading pra ficar espaçoso
                    altura_bloco_texto = 30 + (len(linhas_texto) * 26) + 30
                    ...
                    # Titulo da Barra em Preto Escuro (agora em fonte Bold)
                    d.text((margem_esquerda + 20, y_atual + 8), titulo, fill=COR_CABE_TEXTO, font=fonte_secao)
                    
                    # Imprimir as linhas de texto
                    y_texto_interno = y_atual + 65
                    for linha in linhas_texto:
                        if ":" in linha and not linha.startswith(" "):
                            partes = linha.split(":", 1)
                            # Desenha parte antes dos ":" com fonte bold e depois normal
                            d.text((margem_esquerda + 20, y_texto_interno), partes[0] + ":", fill=COR_CABE_TEXTO, font=fonte_label)
                            ...
                            d.text((margem_esquerda + 20, y_texto_interno), linha, fill=COR_CABE_TEXTO, font=fonte_label)
                        else:
                            d.text((margem_esquerda + 20, y_texto_interno), linha, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                            
                        y_texto_interno += 26 # Um pouco mais de espaço entre as linhas
                        
                    y_atual += 40 + altura_bloco_texto + 40 # Avança mais espaçado para prxs sessão
