...
def carregar_dados():
    ...
    return {
        ...
        'carretas_inicio': 0, 'carretas_fim': 0
    }
...
def salvar_dados():
    ...
    dados_para_salvar = {
        ...
        'carretas_inicio': st.session_state.get('carretas_inicio', 0),
        'carretas_fim': st.session_state.get('carretas_fim', 0)
    }
...
def init_session_state():
    ...
    if 'carretas_inicio' not in st.session_state:
        st.session_state['carretas_inicio'] = dados_salvos.get('carretas_inicio', 0)
    if 'carretas_fim' not in st.session_state:
        st.session_state['carretas_fim'] = dados_salvos.get('carretas_fim', 0)
...
def main():
    ...
    with tab1:
        ...
        with st.form("form_fornecedores", clear_on_submit=True):
            ...
            if submit_fornecedores:
                if nome:
                    # Lógica de Agrupamento Crítica
                    # SE já existe, soma. SENÃO, cria novo.
                    if nome in st.session_state['fornecedores']:
                        st.session_state['fornecedores'][nome]['paletes'] += paletes
                        st.session_state['fornecedores'][nome]['checklists'] += checklists
                    else:
                        st.session_state['fornecedores'][nome] = {
                            'paletes': paletes,
                    ...
                    st.error("Por favor, insira o nome do fornecedor.")
        ...
        if st.session_state['fornecedores']:
            ...
            for f, dados in list(st.session_state['fornecedores'].items()):
                col1, col2 = st.columns([4, 1])
                with col1:
                    st.write(f"- **{f}**: {dados['paletes']} paletes | {dados['checklists']} checklists")
                with col2:
                    # Botão para excluir o fornecedor, usando a chave única do nome
                    if st.button("❌", key=f"del_{f}"):
                        del st.session_state['fornecedores'][f]
                        salvar_dados() # <--- ADICIONE ESTA LINHA AQUI
                        st.rerun()
    ...
    with tab4:
        ...
        with col_inicio:
            ...
            c_inicio = st.number_input("Carretas (Início)", min_value=0, step=1, value=st.session_state['carretas_inicio'])
        with col_fim:
            ...
            c_fim = st.number_input("Carretas (Final)", min_value=0, step=1, value=st.session_state['carretas_fim'])
    ...
    with tab5:
        ...
        if st.button("🚀 Gerar e Copiar Relatório Final", type="primary", use_container_width=True):
            ...
            if st.session_state['fornecedores']:
                ...
                for f, d in st.session_state['fornecedores'].items():
                    relatorio += f"▫️ {f}: {d['paletes']} paletes | {d['checklists']} cklts Feitos\n"
            ...
            relatorio += f"▫️ Carretas\n"
            relatorio += f"   Início: {st.session_state['carretas_inicio']}\n"
            relatorio += f"   Final: {st.session_state['carretas_fim']}\n\n"
            ...
            if HAS_PIL:
                ...
                # Vamos baixar uma fonte segura que suporte acentos e substituir a default boxada.
                @st.cache_data
                def get_font(size):
                    try:
                ...
                fonte_titulo_grande = get_font(36)
                fonte_subtit = get_font(20)
                fonte_secao = get_font(22)
                fonte_label = get_font(16)
                fonte_texto = get_font(16)
                fonte_rodape = get_font(14)

                # Paleta de Cores (Inspirada no modelo)
                COR_AZUL_ESCURO = (15, 60, 180)    # Fundo cabeçalho topo
                COR_AZUL_CLARO = (175, 210, 245)   # Fundo base do documento
                COR_DEST_BARRA = (50, 100, 220)    # Barras de título de sessão
                ...
                if st.session_state['fornecedores']:
                    for f, d in st.session_state['fornecedores'].items():
                        txt_forn += f"• {f}: {d['paletes']} Paletes | {d['checklists']} Checklists Feitos\n"
                else:
                ...
                if st.session_state['divergencias']:
                    for item in st.session_state['divergencias']:
                        ...
                        # Wrap do motivo (para caber na largura da caixinha clara)
                        m_wrap = textwrap.wrap(f"  > Motivo: {item['Motivo']}", width=70)
                        txt_div += "\n".join(m_wrap) + "\n"
                else:
                    txt_div += "Nenhuma.\n"
                ...
                txt_pen += f"• Carretas\n"
                txt_pen += f"  Inicio: {st.session_state['carretas_inicio']}\n"
                txt_pen += f"  Final: {st.session_state['carretas_fim']}\n"
                if st.session_state['pendencias_turno'].strip():
                    txt_pen += f"\n• Anotações Gerais:\n"
                    # Wrap do texto livre
                ...
                for titulo_sessao, texto_sessao in sessoes:
                    ...
                    # Matemática corrigida para casar com o renderizador visual:
                    # 40 (espaço) + 20 (padding up) + (linhas*22) + 20 (padding down) + 30 (espaço bottom)
                    altura_conteudo += 110 + (len(linhas) * 22) 
                
                altura_total = 250 + altura_conteudo + 80 # Cabeçalho(250) + conteudo + Rodape(80)
                LARGURA_IMG = 850
                ...
                # ===============================
                # DESENHANDO CABEÇALHO SUPERIOR (Azul Escuro)
                # ===============================
                d.rectangle([(0, 0), (LARGURA_IMG, 140)], fill=COR_AZUL_ESCURO)
                
                # Textos cabeçalho
                centro_x = LARGURA_IMG // 2
                # Ocultar o ícone da estrela pois algumas fontes "default" falham com ele
                d.text((40, 40), "ALMOXARIFADO", fill=COR_BRANCO, font=fonte_secao)
                d.text((centro_x, 30), "Passagem de Turno", fill=COR_BRANCO, font=fonte_titulo_grande, anchor="ma")
                d.text((centro_x, 80), "RELATÓRIO DIÁRIO DE ATIVIDADES", fill=COR_BRANCO, font=fonte_subtit, anchor="ma")
                d.text((LARGURA_IMG - 40, 40), f"Gerado em: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", fill=COR_BRANCO, font=fonte_rodape, anchor="ra")
                
                # Faixa Info (Cápsulas brancas estilo Forms debaixo do cabeçalho)
                y_capsula = 160
                
                # Pillow mais antigo/básico não suporta 'radius' em .rectangle().
                # Vamos usar .rounded_rectangle() se disponível, senão usamos retângulos normais protegidos por Try/Except 
                try:
                    d.rounded_rectangle([(40, y_capsula), (380, y_capsula+45)], fill=COR_BRANCO, radius=8)
                    d.rounded_rectangle([(400, y_capsula), (600, y_capsula+45)], fill=COR_BRANCO, radius=8)
                    d.rounded_rectangle([(620, y_capsula), (LARGURA_IMG-40, y_capsula+45)], fill=COR_BRANCO, radius=8)
                except AttributeError:
                    d.rectangle([(40, y_capsula), (380, y_capsula+45)], fill=COR_BRANCO)
                    d.rectangle([(400, y_capsula), (600, y_capsula+45)], fill=COR_BRANCO)
                    d.rectangle([(620, y_capsula), (LARGURA_IMG-40, y_capsula+45)], fill=COR_BRANCO)
                ...
                # Formata exatamente como o dia, sem o prefixo
                dia_semana_str = dias_pt[int(datetime.datetime.now().strftime('%w'))]
                
                d.text((55, y_capsula+12), f"Dia da Semana: {dia_semana_str}", fill=COR_TEXTO_ESCURO, font=fonte_label)
                d.text((415, y_capsula+12), f"Hora: {datetime.datetime.now().strftime('%H:%M')}", fill=COR_TEXTO_ESCURO, font=fonte_label)
                d.text((635, y_capsula+12), "Sistema: Web App", fill=COR_TEXTO_ESCURO, font=fonte_label)

                # ===============================
                # DESENHANDO SESSÕES E LISTAS
                # ===============================
                y_atual = 240
                margem_esquerda = 40
                ...
                for titulo, linhas_texto in linhas_sessao:
                    # Bloco Fundo da sessão (branco)
                    altura_bloco_texto = 20 + (len(linhas_texto) * 22) + 20
                    # Sombras / Borda Retângulo Banco
                    d.rectangle([(margem_esquerda, y_atual + 40), (margem_esquerda + largura_quadro, y_atual + 40 + altura_bloco_texto)], fill=COR_BRANCO)
                    
                    # Faixa Titulo da Sessão (Azul Medio sob o branco)
                    d.rectangle([(margem_esquerda, y_atual), (margem_esquerda + largura_quadro, y_atual + 40)], fill=COR_DEST_BARRA)
                    d.text((margem_esquerda + 20, y_atual + 8), titulo, fill=COR_BRANCO, font=fonte_secao)
                    
                    # Imprimir as linhas de texto
                    y_texto_interno = y_atual + 60
                    for linha in linhas_texto:
                        # Negrito no inicio ajuda leitura (Cód / Paletes) 
                        if ":" in linha and not linha.startswith(" "):
                            partes = linha.split(":", 1)
                            # Desenha parte antes dos ":" com fonte mais grossa (label)
                            d.text((margem_esquerda + 20, y_texto_interno), partes[0] + ":", fill=COR_TEXTO_ESCURO, font=fonte_label)
                            # Pega comprimento da string pra dar espaco
                            tam_label = d.textlength(partes[0] + ": ", font=fonte_label)
                            d.text((margem_esquerda + 20 + tam_label, y_texto_interno), partes[1], fill=COR_TEXTO_ESCURO, font=fonte_texto)
                        else:
                            d.text((margem_esquerda + 20, y_texto_interno), linha, fill=COR_TEXTO_ESCURO, font=fonte_texto)
                            
                        y_texto_interno += 22
                        
                    y_atual += 40 + altura_bloco_texto + 30 # Avanca para prxs sessão

                # Base (Assinatura do sistema)
                try:
                    d.rounded_rectangle([(margem_esquerda, altura_total-60), (LARGURA_IMG-margem_esquerda, altura_total-20)], fill=COR_BRANCO, radius=10)
                except AttributeError:
                    d.rectangle([(margem_esquerda, altura_total-60), (LARGURA_IMG-margem_esquerda, altura_total-20)], fill=COR_BRANCO)
                    
                d.text((margem_esquerda + 20, altura_total-45), "Relatório submetido via App Passagem de Turno Almoxarifado ©", fill=COR_TEXTO_ESCURO, font=fonte_rodape)
            ...
            writer.writerow(['Data_Geração', 'Sessão', 'Identificação (Fornec/Cód)', 'Produto_Nome', 'Lote', 'Qtd_Paletes', 'Checklists_Feitos', 'Anotações_Motivos'])
            
            data_hora_str = datetime.datetime.now().strftime('%d/%m/%Y %H:%M')
            
            for f, d in st.session_state['fornecedores'].items():
                writer.writerow([data_hora_str, 'FORNECEDOR', f, '', '', d['paletes'], d['checklists'], ''])
